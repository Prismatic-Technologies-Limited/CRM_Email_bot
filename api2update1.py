...
class EmailAutomationService:
    ...
    async def send_to_external_api(self, email: str, comments: str, user_type: str = "customer", phone: str = "", lead_id: str = "") -> bool:
        ...
        try:
            ...
            response = requests.post(
                self.api_endpoint,
                json=payload,
                headers=headers,  # This now includes the Bearer token
                timeout=30
            )
...
Prismatic Technologies"""
            ...
            #comments = f"OUTGOING WELCOME EMAIL - Subject: {email_subject} | Body: {email_body[:500]}... | Product Interest: {product_interest} | Delivery Status: {'Sent successfully' if email_sent else 'Failed to send'}"
            comments = f"{email_body[:500]}"
    ...
    async def process_incoming_email(self, email_data: Dict) -> Dict:
        ...
        try:
            ...
            if generated_response and sendable:
                ...
                logger.info(f" Not sending reply - Generated: {bool(generated_response)}, Sendable: {sendable}")
    ...
    #         comments_parts = [f"OUTGOING EMAIL - Sent: {timestamp}", f"Subject: {subject}"]
    ...
    async def post_outgoing_email_to_api(
        ...
        try:
            clean_email = self.extract_email_from_sender(recipient_email)
            if not clean_email:
                logger.warning("⚠️ No valid recipient email extracted.")
                return False
            ...
            logger.info(f"📤 Outgoing Email Comment: {comments}")
            ...
            logger.info(f"✅ API post result: {result}")
            return result
    ...
    def monitor_emails_background(self):
        ...
        while monitoring_state["active"]:
            try:
                ...
                logger.info(f" Email check completed. Next check in 60 seconds. Stats: Processed: {monitoring_state['emails_processed']}, API Posts: {monitoring_state['api_posts_count']}, API Failures: {monitoring_state['api_failures']}")
                time.sleep(21600)  # Check every 60 seconds
